...
- Import Apple Santé entièrement en flux (streaming), dans un Web Worker (`src/workers/appleHealthWorker.ts`) : `fflate.Unzip` décompacte le `.zip` par morceaux et restitue le XML décompressé par morceaux via son callback `ondata`, jamais comme une seule chaîne complète. Un scanner regex incrémental (`AnalyseurSanteIncremental` dans `src/utils/appleHealthParser.ts`) traite chaque morceau au fil de l'eau, ne gardant en mémoire qu'un petit reliquat (une balise coupée pile à la frontière entre deux morceaux). C'est indispensable pour un long historique Apple Watch : un `.zip` de quelques centaines de Mo peut se décompresser en plusieurs Go de XML (données très répétitives), largement au-delà de ce qu'un téléphone peut tenir en une seule chaîne JavaScript — d'où l'architecture en flux plutôt qu'un simple découpage a posteriori. Une vraie barre de progression accompagne l'analyse. Voir `src/utils/appleHealthImport.ts` pour l'API côté thread principal (lit le fichier, lance le Worker, relaie la progression).
