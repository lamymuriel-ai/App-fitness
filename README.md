...
- Le récap visuel (`src/components/RecapVisuel.tsx`, logique dans `src/utils/recapJournalier.ts`) calcule un statut par jour et par métrique : sommeil ok si ≥ 7h, pas ok si ≥ `objectifPas`, calories ok si dans une tolérance de ±15 % de l'objectif, séance ok si celle prévue ce jour-là (`PLANNING_SEMAINE`) est marquée terminée. Une métrique sans donnée du tout ce jour-là est "absente" (gris), distincte d'un objectif manqué (orange) — l'appli ne pénalise jamais l'absence de saisie, seulement un écart mesuré. Le statut global du jour ne tient compte que des métriques qui ont une donnée (+ la séance, sauf jour de repos) : vert si toutes sont dans les clous, orange si au moins une ne l'est pas, gris seulement si rien du tout n'a été renseigné ce jour-là.
