...
- Import Apple Santé via `fflate` (décompression du `.zip` côté client) et analyse du XML par expressions régulières (évite de charger un DOM complet pour de gros fichiers). Tout le travail lourd (décompression + balayage regex) tourne dans un Web Worker (`src/workers/appleHealthWorker.ts`) plutôt que sur le fil principal, avec une vraie barre de progression — indispensable pour les exports volumineux d'un long historique Apple Watch (potentiellement plusieurs centaines de Mo), qui sinon figeraient l'interface sans retour visuel. Voir `src/utils/appleHealthParser.ts` pour la logique de parsing et `src/utils/appleHealthImport.ts` pour l'API côté thread principal.
